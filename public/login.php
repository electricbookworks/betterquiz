...
if ("login / register"==$act) {
	...
	if (!$ok) {
		if ("bad-password"==$userId) {
			new Flash("Sorry, the password is incorrect.", "error");
			Utils::Redirect("login_form.php", 
				array("qz"=>$qz, "email"=>$email));
		}
		new Flash("Sorry, I don't recognize that " . (Utils::IsEmail($email) ? "email" : "cell number") . ". You can register here.", "error");
		$p = array("qz"=>$qz);
	...
	if ($user->IsMerged()) {
		...
		new Flash("Sorry, that user account has been merged with " .
